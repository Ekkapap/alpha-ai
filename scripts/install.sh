...
# Add alpha binary dir to PATH — write to ~/.alpha-ai/env.sh (overwrite on each install),
# then source it from shell rc once. No accumulation in .zshrc.
if [[ "$OS" == "Darwin" ]]; then
  _ALPHA_BIN_DIR="$ALPHA_DIR/agents-resource/tools/bin/darwin"
else
  _ALPHA_BIN_DIR="$ALPHA_DIR/agents-resource/tools/bin/linux"
fi
_ENV_FILE="$HOME/.alpha-ai/env.sh"
mkdir -p "$HOME/.alpha-ai"
_ENV_CONTENT="# alpha-ai — generated by install.sh, do not edit manually
export PATH=\"$_ALPHA_BIN_DIR:\$PATH\""
if [[ "$(cat "$_ENV_FILE" 2>/dev/null)" != "$_ENV_CONTENT" ]]; then
  printf '%s\n' "$_ENV_CONTENT" > "$_ENV_FILE"
  ok "Updated $_ENV_FILE"
else
  ok "env.sh already up to date"
fi
...
_SOURCE_LINE="source \"$HOME/.alpha-ai/env.sh\""
if ! grep -qF "$HOME/.alpha-ai/env.sh" "$_RC" 2>/dev/null; then
  printf '\n%s\n' "$_SOURCE_LINE" >> "$_RC"
  ok "Added source line to $_RC"
  info "Run: source $_RC"
else
  ok "Shell rc already sources env.sh ($_RC)"
fi
