...
func startPipeline() error {
	...
	scanResult := filepath.Join(projectRoot, ".understand-anything/intermediate/scan-result.json")

	// Clean stale batches from previous runs
	os.RemoveAll(filepath.Join(projectRoot, ".understand-anything/intermediate"))
	os.MkdirAll(filepath.Join(projectRoot, ".understand-anything/intermediate"), 0755)
	...
	// 3. Get total batches
	batchesPath := filepath.Join(projectRoot, ".understand-anything/intermediate/batches.json")
...
func updatePipeline() error {
	...
	metaPath := filepath.Join(projectRoot, ".understand-anything/meta.json")
	...
	tmpDir := filepath.Join(projectRoot, ".understand-anything/tmp")
...
func pruneAndPreserveExisting(changedFilesStr string) error {
	graphPath := filepath.Join(projectRoot, ".understand-anything/knowledge-graph.json")
	...
	return os.WriteFile(filepath.Join(projectRoot, ".understand-anything/intermediate/batch-existing.json"), prunedBytes, 0644)
}

func finalizeGraph() error {
	assembledPath := filepath.Join(projectRoot, ".understand-anything/intermediate/assembled-graph.json")
	outGraphPath := filepath.Join(projectRoot, ".understand-anything/knowledge-graph.json")
	outMetaPath := filepath.Join(projectRoot, ".understand-anything/meta.json")
